    ...
    def __call__(self, rclone):
        ...
        if not rclone:
            ...
            if Path(pyrclonec).exists():
                ...
                logger.warning("Could not find rclone in your PATH. Enter it manually " "and the program will remember it.")
...
class Rclone(CheckRclone):
    ...
    def _size_units(self, s, mult):
        if "KiB" in s:
            ...
            s = round(float(s.split(" MiB")[0]) * 1.049e6 / mult, 2)
        elif "GiB" in s:
            s = round(float(s.split(" GiB")[0]) * 1.074e9 / mult, 2)
        return s
        ...
        with tqdm(total=size, unit=self.unit) as pbar:
            ...
            for line in p.stdout:
                ...
                if "Transferred" in s and "ETA" in s:
                    s = s.split("Transferred:")[1].split("\t")[1].lstrip().split(" / ")[0]
    ...
    def _process(self, subcommand, from_, to="", progress=True, _execute=False, *args):
        ...
        if subcommand in ["copy", "move"] and from_ and not to:
            raise MissingDestination("The command requires passing a destination.")
        ...
        if subcommand == "lsjson":
            ...
            match = (
                OUT.split("differences found")[0]
                .split("matching files")[0]
                .split("NOTICE: ")[-1]
                .split(": ")[-1]
                .split("root ")[-1]
                .split(" ")[0]
    ...
    def delete(*args, **kwargs):
        raise NotImplementedError("delete is a protected command! Use `execute()` instead.")
